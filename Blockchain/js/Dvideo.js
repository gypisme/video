(function (window, document) {
	var Dvideo = function (options) {
		...
		this.localValue = {
			...
			ele: '',
			// 地址
			src: 'http://dgtt.besttoptoday.com/bc/LRTEN800K.mp4',
	...
	Dvideo.prototype = {
		...
		exitFullScreenStyle: function () {
			...
			this.updateFullScreenState(false)
			this.opt.ele.style.width = 100 +'%'
			this.opt.ele.style.height = 'auto'
		},
		...
		videoPlay: function () {
			try{
				this.videoEle.play();
				this.isPlaying = true
                console.log(this.isPlaying + '1111111')
			} catch (e) {
				console.log(e)
			}
		},
...
//			}
		...
		createVideoDefinition: function () {
			...
			// active索引
            var videoactive = Number(videoDefinitionData.activeIndex);
            if($(window).width()<960){
                videoactive = 0
            }else if($(window).width()>960){
                videoactive = 1
            }
			var videoDefinitionIndex = videoactive
